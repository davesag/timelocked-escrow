...
contract('TimelockedEscrow (core functionality)', accounts => {
  ...
  context('after successful deposit', () => {
    ...
    context('hasFunds', () => {
      ...
      it('zero address fails', () => assertThrows(escrow.hasFunds(0x0, amount)))
    ...
    context('areFundsTimelocked', () => {
      ...
      it("deadbeat punter's funds are not timelocked", async () => {
        const areFundsTimelocked = await escrow.areFundsTimelocked(
          deadbeatPunter
        )
      ...
      it('zero address fails', () =>
        assertThrows(escrow.areFundsTimelocked(0x0)))
    })
    ...
    context('transferring funds', () => {
      ...
      it("punter can't transfer more key than they have deposited", () =>
        assertThrows(
          escrow.transfer(serviceProvider, amount + 1, { from: punter })
        ))
      ...
      it('punter can transfer to a whitelisted service provider', async () => {
        const tx = await escrow.transfer(serviceProvider, amount, {
          from: punter
        })
